...
clean = lambda s: ''.join(c for c in s.lower() if c in ALPHABET)
...
def make_paragraph(nb_chars=100):
    text = 'b'
    while len(text)<nb_chars:
        if text[-1]=='b':
            text += random.choice('aaaaae')
        elif text[-1]=='g':
            text += random.choice('r ')
        elif text[-1]=='n':
            text += random.choice('ga')
        elif text[-1]=='m':
            text += random.choice(' ')
        elif text[-1]=='a':
            text += random.choice('nmg ')
        elif text[-1]=='e':
            text += random.choice('br')
        elif text[-1]=='r':
            text += random.choice('aaaarg')
        else:
            text += random.choice('aaaaabbbbbegmnr ')

    return clean(text)

if __name__=='__main__':
    TRAIN = [clean(make_paragraph()) for _ in range(500)]
    TEST  = [clean(make_paragraph()) for _ in range(500)]

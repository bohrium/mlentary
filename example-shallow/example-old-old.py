...
all_x = all_x[idxs]
all_y = all_y[idxs]
all_y_sign = np.array([+1 if y==DIG_B else -1 for y in all_y]) 
#
...
make_clsfier = lambda dec_func: lambda x: DIG_A if dec_func(x)<=0 else DIG_B 
...
#def gradient_of_hinge_loss_wrt_weight(A,B, idxs=train_idxs): 
def gradient(A,B, y_sign,x): 
...
def gradients(A,B, idxs=train_idxs): 
    gs = [gradient(A,B,all_y_sign[i],all_x[i]) for i in idxs]
    gAs = [g[0] for g in gs] 
    gBs = [g[1] for g in gs] 
    ls  = [g[2] for g in gs] 
    ...
    if dec_func is not None:
        for r in range(data_h):
            for c in range(data_w):
                ...
                dec = dec_func((z0,z1,z2)) #q0*z0+q1*z1+q2*z2
...
def gradient_descend(learning_rate, nb_steps, report_every=None):
    H = 8 
    A = np.random.randn(H)#/np.sqrt(1+H)
    B = np.random.randn(H,3)/np.sqrt(H+3)
    for t in range(nb_steps):
        #
        batch = np.random.choice(train_idxs, 32, replace=False)
        gA, gB, l = gradients(A,B, train_idxs)
        ...
        print('{:6d} {:6.3f} {:5.2f} {:5.2f}'.format(t, l,
            error_rate(clsfier, train_idxs),
            error_rate(clsfier, test_idxs ) ))
        #
        plot_features(idxs=train_idxs, dec_func=dec_func,
                interesting_params = B, ip_weights = A,
                file_name='woah-{:02d}-{:04d}.png'.format(H,t), opacity_factor=0.75) 

print('hey')
#gradient_descend(0.25 , 500+1, 25)
gradient_descend(0.25 , 5000+1, 250)
